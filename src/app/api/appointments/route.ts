...
export async function POST(req: Request) {
  ...
  if (!rl.ok) {
    return NextResponse.json(
      { ok: false, error: "చాలా ఎక్కువ రిక్వెస్ట్‌లు. దయచేసి రిసెప్షన్‌కు కాల్ చేయండి. Too many requests — please call our reception." },
      { status: 429 }
    );
  }
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { ok: false, error: "దయచేసి ఫారం ఫీల్డ్‌లను తనిఖీ చేసి మళ్లీ ప్రయత్నించండి. Please check the form fields and try again." },
      { status: 400 }
    );
  }
  ...
  if (!dept) {
    return NextResponse.json({ ok: false, error: "గుర్తుపెట్టని శాఖ / Unknown department." }, { status: 400 });
  }
  if (data.doctorSlug) {
    const doc = doctors.find((d) => d.slug === data.doctorSlug);
    if (!doc) {
      return NextResponse.json({ ok: false, error: "గుర్తుపెట్టని వైద్యుడు / Unknown doctor." }, { status: 400 });
    }
  }
  ...
  if (Number.isNaN(requested.getTime()) || requested < today) {
    return NextResponse.json({ ok: false, error: "తేదీ ఈ రోజు లేదా తర్వాత ఉండాలి / Date must be today or later." }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "రిక్వెస్ట్ సేవ్ చేయలేకపోతున్నాము — దయచేసి ఆసుపత్రికి కాల్ చేయండి. Could not save your request — please call the hospital." },
      { status: 500 }
    );
  }
}
